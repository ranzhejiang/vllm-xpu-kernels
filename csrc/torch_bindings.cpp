...
TORCH_LIBRARY_EXPAND(TORCH_EXTENSION_NAME, ops) {
  ...
  ops.def("gelu_and_mul(Tensor! out, Tensor input, Tensor? valid_rows=None) -> ()");
  ops.impl("gelu_and_mul", torch::kXPU, &gelu_and_mul);

  ops.def("gelu_tanh_and_mul(Tensor! out, Tensor input, Tensor? valid_rows=None) -> ()");
  ops.impl("gelu_tanh_and_mul", torch::kXPU, &gelu_tanh_and_mul);

  ops.def("fatrelu_and_mul(Tensor! out, Tensor! input, float threshold) -> ()");
  ops.impl("fatrelu_and_mul", torch::kXPU, &fatrelu_and_mul);

  ops.def("gelu_fast(Tensor! out, Tensor input, Tensor? valid_rows=None) -> ()");
  ...
  ops.def("gelu_quick(Tensor! out, Tensor input, Tensor? valid_rows=None) -> ()");
  ...
  // relu2_no_mul
  ops.def("relu2_no_mul(Tensor! out, Tensor! input, Tensor? valid_rows=None) -> ()");
  ops.impl("relu2_no_mul", torch::kXPU, &relu2_no_mul);

  // swiglustep_and_mul
  ops.def(
      "swiglustep_and_mul(Tensor! out, Tensor input, float limit=7.0, Tensor? valid_rows=None) "
      "-> ()");
